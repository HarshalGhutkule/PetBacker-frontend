...
export const Home = () => {
  ...
  const searchBlock = (e)=>{
    ...
    if(e.target.value !== ""){
      axios.get(`https://petbacker-backend.onrender.com/services/city/${e.target.value}`,{
        headers: {
  ...
  const sorting = (a)=>{
    ...
    axios.get(`https://petbacker-backend.onrender.com/services?page=${pageof}&size=5&sort=${a}`,{
      headers: {
        'Authorization': `Bearer ${token}` 
      }}).then((res)=>{
  ...
  const getData = (pageof) => {
    axios.get(`https://petbacker-backend.onrender.com/services?page=${pageof}&size=5`,{
      headers: {
        'Authorization': `Bearer ${token}` 
      }}).then((res)=>{
      setData(res.data.service);
